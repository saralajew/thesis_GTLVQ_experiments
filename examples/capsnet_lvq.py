...
The network is equivalent to CapsNet [Sabour et al.] except the use of a LVQ-Capsule.
...
from anysma.modules.transformation import LinearTransformation
...
def LvqCapsNet(input_shape, n_class):
    ...
    digitcaps = layers.Conv2D(filters=256, kernel_size=9, strides=1, padding='valid', activation='relu')(x)
    digitcaps = layers.Conv2D(filters=8 * 32, kernel_size=9, strides=2, padding='valid')(digitcaps)

    # LVQ-Capsule
    digit_caps = Capsule(name='digitcaps', prototype_distribution=(1, n_class))
    digit_caps.add(InputModule(signal_shape=8, init_diss_initializer='zeros'))
    digit_caps.add(LinearTransformation(output_dim=16, scope='local'))
    diss = MinkowskiDistance(prototype_initializer='zeros', squared_dissimilarity=False, name='minkowski_distance')
    digit_caps.add(diss)
    digit_caps.add(GibbsRouting(norm_axis='channels', trainable=False,
                                diss_regularizer='max_value'))
    digit_caps.add(DissimilarityTransformation(probability_transformation='neg_exp', name='lvq_caps'))
...
def train(model, data, args):
    ...
    if args.pre_training:
        # pre-training over some random samples
        idx = np.random.randint(0, 60000 - 1, (10000,))
        diss = model.get_layer('minkowski_distance')  # get the diss layer
        pre_train_model = models.Model(model.input[0], diss.input[0])
        diss_input = pre_train_model.predict(x_train[idx, :], batch_size=args.batch_size)

        diss.pre_training(x_train=diss_input, y_train=y_train[idx], capsule_inputs_are_equal=False)
...
def plot_prototypes(model, decoder, args):
    print('-' * 30 + 'Begin: plot prototypes' + '-' * 30)
    proto_layer = model.get_layer('minkowski_distance')
    ...
    image = img * 255
    Image.fromarray(image.astype(np.uint8)).save(args.save_dir + '/protos_reconstructed.png')
    print('plot prototypes result saved to %s/protos_reconstructed.png' % args.save_dir)

    print('-' * 30 + 'End: plot prototypes' + '-' * 30)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="LVQ Capsule Network on MNIST.")
    parser.add_argument('--epochs', default=50, type=int)
    parser.add_argument('--batch_size', default=128, type=int)
    parser.add_argument('--lr', default=0.005, type=float,  # 005; 001
                        help="Initial learning rate.")
    parser.add_argument('--lam_recon', default=0.392, type=float,  # 392
    ...
    parser.add_argument('--digit', default=5, type=int,
                        help="Digit to manipulateduring test.")
    parser.add_argument('-w', '--weights', default=None,
                        help="The path of the saved weights. Should be specified when testing.")
    parser.add_argument('-p', '--pre_training', action='store_true',
                        help="Initialize the prototypes via pre-training.")
    ...
    # define model
    model, eval_model, manipulate_model, decoder = LvqCapsNet(input_shape=x_train.shape[1:],
    ...
    if not args.testing:
        ...
        plot_prototypes(model, decoder, args)

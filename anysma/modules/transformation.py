...
from ..utils.caps_utils import mixed_shape, equal_int_shape
...
class LinearTransformation(Module):
    def __init__(self,
        ...
        # no diss regularizer, because no adaption/ transformation
    ...
    def _build_sparse(self, input_shape):
        if self.scope in ('local', 'capsule_wise'):
            raise ValueError('If the signal is sparse, the scope `local` and `capsule_wise` is not supported.')

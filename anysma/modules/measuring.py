...
class RestrictedTangentDistance(PointMatrixPrototype):
    def __init__(self,
        ...
        # no support for sparse signals; right now there is no efficient implementation. The only possible one is
        # with heavily used map_fn which is too slow and not desirable. Further, it is more a final layer and hence the
        # signal should be small enough
        self.support_sparse_signal = False

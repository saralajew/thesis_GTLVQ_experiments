...
# all implementations should test against check shapes at the beginning.
...
def restricted_tangent_distance(signals, protos, subspaces, bounds,
    ...
    # shape(subspaces): (optional [proto_number]) x prod(dim1 * dim2 * ... * dimN)  x prod(projected_atom_shape)
    #
    # no solution for sparse signals without a heavy use of map_fn. Thus, not supported so far.
    ...
    with K.name_scope('tangent_distance'):
        ...
        if K.ndim(subspaces) == 2:
            ...
            with K.name_scope('tangentspace_projections'):
                ...
                projected_diff = K.reshape(projected_diff,
                                           (signal_shape[0], signal_shape[2], proto_shape[0]) + signal_shape[3:])

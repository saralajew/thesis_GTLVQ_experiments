...
Section 3.4.2 for the GTLVQ and GTLVQ-1M model.
"""
...
def plot_random_samples(model, protos_per_class, path):
    ...
    for i in range(10):
        for j in range(protos_per_class):
            ...
            for k in range(trails):
                ...
                proto = np.reshape(proto, (28, 28, 1))
...
def plot_best_approximating_prototype(model, protos_per_class, x_test, path):
    if not os.path.exists(path):
        os.mkdir(path)
    ...
    for k in range(trails):
        for i in range(10):
            for j in range(protos_per_class):
                ...
                theta = np.dot(B[idx, :].transpose(),
                               x_test[k].reshape(28 * 28, ) - t[idx, :])
                proto = t[idx, :] + np.dot(B[idx, :], theta)
                proto = np.reshape(proto, (28, 28, 1))
                ...
                cv2.imwrite('{}/proto_{}_{}_{}.png'.format(path, i, j, k),
                            (proto * 255).astype('uint8'))
        # plot sample
        cv2.imwrite('{}/sample_{}.png'.format(path, k),
                    (np.repeat(x_test[k], 3, 2) * 255).astype('uint8'))
...
if __name__ == '__main__':
    ...
    plot_random_samples(train_model, protos_per_class,
                        args.save_dir + '/random_samples')
    plot_best_approximating_prototype(train_model, protos_per_class, x_test,
                                      args.save_dir + '/best_approximation')
